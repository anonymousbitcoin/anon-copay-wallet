...
angular
  ...
  .factory("storageService", function(
    ...
    root.setMasternode = function(network, masternode, cb) {
      storage.set("masternode-" + network, masternode, cb);
    };

    root.getMasternode = function(network, cb) {
      storage.get("masternode-" + network, cb);
    };

    root.removeMasternode = function(network, cb) {
      storage.remove("masternode-" + network, cb);
    };
    ...
    root.getBitpayAccounts = function(network, cb) {
      storage.get("bitpayAccounts-v2-" + network, function(
        err,
        allAccountsStr
      ) {
        if (err) return cb(err);
        ...
        lodash.each(allAccounts, function(account, email) {
          ...
          if (!account.token && account["bitpayApi-" + network].token) {
            ...
            if (!account.cards) {
              account.cards = lodash.clone(
                account["bitpayDebitCards-" + network]
              );
            }
    ...
    root.removeBitpayAccount = function(network, account, cb) {
      ...
      storage.get("bitpayAccounts-v2-" + network, function(
        ...
        storage.set(
          "bitpayAccounts-v2-" + network,
          JSON.stringify(bitpayAccounts),
          cb
        );
      });
    };
