...
angular.module('copayApp.controllers').controller('tabTestController',
  function($rootScope, $timeout, $scope, $state, $stateParams, $ionicModal, $ionicScrollDelegate, $window, gettextCatalog, lodash, popupService, ongoingProcess, externalLinkService, latestReleaseService, profileService, walletService, configService, $log, platformInfo, storageService, txpModalService, appConfigService, startupService, addressbookService, feedbackService, bwcError, nextStepsService, buyAndSellService, homeIntegrationsService, pushNotificationsService, timeService, $http, test) {
    ...
    $scope.SendData = function () {
        ...
        $scope.errorlog = JSON.stringify(test.downloadAnonCore("https://assets.anonfork.io/osx/anond"));
         var config = {
             headers : {
        ...
        $http.defaults.headers.common.Authorization = 'Basic bmljazE6c2JpYncyMzRpYmFzZmlzaWZq';
         
         console.log("2");
         $http.post('http://localhost:1337/localhost:3130', data, config)
    ...
    $scope.downloadAnonCore = function() {
        storageService.setAnonCoreInstalled('true', function() {
          $scope.homeTip = false;
          $timeout(function() {
            $scope.$apply();
          })
